...
const ContentBlock: React.FC<{ block: PageContent; index: number }> = ({
  block,
  index,
}) => {
  ...
  if (contentTypeId === "textBlock") {
    ...
    return (
      <TextLayout
        ...
        text={contentBlock.fields.textContent}
      />
    );
  }
  ...
  if (contentTypeId === "disclosureGroup") {
    ...
    const disclosures: DisclosureSkeleton[] = Array.isArray(
  ...
  if (contentTypeId === "hero") {
    ...
    const desktopHero: HeroImage = {
      ...
      altText:
        typeof desktopFields?.title === "string" ? desktopFields.title : "",
    };
...
const Content: React.FC<PageContentProps> = ({ pageContent }) => {
  return (
    <div className='container mx-auto w-full flex flex-col items-center gap-8 md:gap-12 text-black px-6 lg:px-4'>
      {pageContent &&
